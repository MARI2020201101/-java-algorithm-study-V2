...
public class KiwiJuiceEasyV5 {
    ...
    public static int[] thePouring(int[] capacities , int[] bottles, int[] fromId, int[] toId){
        ...
        for (int i = 0; i < fromId.length; i++) {
            int totalJuice = bottles[fromId[i]] + bottles[toId[i]];
            bottles[toId[i]] = Math.min(totalJuice, capacities[toId[i]]);
            //총량과 capacity 중에 더 적은 양이 toIdBottle에 들어가게 되고
            //fromIdBottle 에는 남은 양이 들어가게 된다.
            bottles[fromId[i]] = totalJuice - bottles[toId[i]];
            // 총량을 보존해야 한다.
        }
       return bottles;
    }
